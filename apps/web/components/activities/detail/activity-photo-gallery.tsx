...
function photoLabel(photo: ActivityPhoto, index: number): string {
	if (index === 0) return "Overview";
	return cleanLabel(photo.alt) ?? `Photo ${index + 1}`;
}

function galleryStops(photos: ActivityPhoto[]): GalleryStop[] {
	const stops: GalleryStop[] = [];
	const seenLabels = new Set<string>();

	for (const [index, photo] of photos.entries()) {
		const label = photoLabel(photo, index);
		const normalized = cleanLabel(photo.alt)?.toLowerCase();
...
function imageAlt(photo: ActivityPhoto, title: string, index: number): string {
	return photo.alt ?? `${title} photo ${index + 1}`;
}
			...
			>
				<div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6">
					...
					<Button
						...
						aria-label={copied ? "Activity link copied" : "Share this activity"}
					>
						{copied ? (
